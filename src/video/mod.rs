...
impl Renderer {
    pub fn new() -> Self {
        ...
        unsafe {
            ...
            gl::BindBuffer(gl::ELEMENT_ARRAY_BUFFER, texture_uv_vbo);
            gl::BufferData(gl::ELEMENT_ARRAY_BUFFER, 0, std::ptr::null(), gl::DYNAMIC_DRAW);
            gl::VertexAttribPointer(1, 2, gl::FLOAT, gl::FALSE, std::mem::size_of::<Coordinate2D>() as i32, std::ptr::null());
            gl::EnableVertexAttribArray(1);
        }

        Self { texture_shader, texture_vao, texture_vbo, texture_ebo, texture_uv_vbo }
    }

    pub fn blit(&self, surface: &mut Surface, pos_x: f32, pos_y: f32, width: f32, height: f32) {
        #[rustfmt::skip]
        let VERTICES: [Coordinate2D; 4] = [
            Coordinate2D(pos_x, pos_y),
            Coordinate2D(pos_x, pos_y + height),
            Coordinate2D(pos_x + width, pos_y + height),
            Coordinate2D(pos_x + width, pos_y),
        ];
        ...
        let UVS: [Coordinate2D; 4] = [
            Coordinate2D(0.3, 1.0),
            Coordinate2D(0.8, 1.0),
            Coordinate2D(1.0, 0.2),
